...
<div style="text-align: center; font-weight: bold; font-size: xx-large">I Don't Use Python or R </div>
<div style="text-align: center; font-weight: bold; font-size: xx-large">-</div>
<div style="text-align: center; font-weight: bold; font-size: xx-large">Why Should I Use Jupyter Notebooks?</div>

__<div style="text-align: center; font-weight: normal; font-size: x-large">Jack Hamilton</div>__

__<div style="text-align: center;">Division of Research__ 

__<div style="text-align: center;">Kaiser Permanente__

__<div style="text-align: center;">Oakland, California__</div> 
...
This paper was written using the markdown document facility in Jupyter; markdown is one of the topics of this paper.  As a consequence of using markdown instead of Word, it will not be possible to make this paper look like all the other papers. 

One thing I didn't completely realize about Jupyter until I wrote this presentation is what a tremendous time-saver it is to use markdown instead of Microsoft Word.  It saved at least a day of work.  Besides the time savings in the initial creation, using markdown makes updates much easier - no need to copy and paste code and results from SAS to Word, no worries about whether I've accidentally updated a piece of code without updating all the results...  It happens automatically!
...
- You can break a program up into small pieces and rerun those pieces in any order, without losing the history of what you've done. That's good for iterative development. The SAS program, output, and log are shown inline; that's integrated output.

- You can easily include documentation in the notebook using a simple but powerful markup language named _markdown_.  It is easy to include common document constructs such as headings and tables. That's stylish (or rich-text) documentation. 
...
Why should you care about notebooks?  Maybe you shouldn't.  You might not need or want anything Jupyter does, or you might find it slow and cumbersome (the way I feel about Enterprise Guide, which many people claim to like), or you may have existing processes that are time-tested and comfortable and don't need to be changed.  But I've found it very useful, especially for new small-ish projects that need to be presented to others.  Having the programs and documentation is one place is a big time-saver.
...
___workflows leveraging__ the simple yet ___presentationally elegant___ Jupyter Notebook ___coding and publication
environment___, along with the broader Python ___data science ecosystem___ that comes with it.  [https://support.sas.com/content/dam/SAS/support/en/sas-global-forum-proceedings/2018/2822-2018.pdf](https://support.sas.com/content/dam/SAS/support/en/sas-global-forum-proceedings/2018/2822-2018.pdf)

(I don't mean to pick on the author of that paper - it's actually a good paper, and I encourage you to read it.  But the introduction is Buzzword City, and it doesn't tell you what notebooks are used for.)
...
>Something I learned about at WUSS, and have not yet figured out how to incorporate into this document, is tab completion.  In SAS Studio, you can turn on hints to find the syntax of the SAS statements you use.  In Jupyter, you can get something very similar by pressing the TAB key.
...
After the code has completed running, you will see two types of cells.  The cell with text in a grey box (labeled with a blue 1 at the left) is a code cell, and the cell underneath (with a red 1) is an output cell that shows the log resulting from running the code in the code cell.  The code cell is editable, and as you can see, it has syntax coloring.

Like SAS Studio and Enterprise Guide, Jupyter sends code off into the ether to run.  Generally it will go to a server somewhere.  The first thing to happen is establishing a connection, as shown in ``SAS Connection established. Subprocess id is 16416``.  There's a 10-20 second pause while communications are being established, but that happens only once at the beginning.  Jupyter will keep re-using the same connection until you close it.
...
One of the best features of Jupyter is its support for "markdown", which is a text-based format for rich text display.  This presentation was written in markdown.

What I like about markdown:

- It's easy to write, because ease of writing was one of the design goals.

- It looks like plain text, because it mostly is plain text.  Markdown elements that tell Jupyter how to display the documentation are just text, and are not hard to understand even if you don't have a markdown viewer.
...
I don't have Git on this machine, but this extension works well on a machine that does.  See https://github.com/jupyterlab/jupyterlab-git for an example in the form of an animated GIF.
...
```
    ...
    Please enter the IOM user id:  c449630
    Please enter the password for IOM user :  ·········
    

    SAS Connection established. Subprocess id is 9092
    ...
    Time: 35.572609186172485 seconds.
...
<div class="highlight"><pre><span></span><span class="err">4</span> <span class="err">The</span> <span class="err">SAS</span> <span class="err">System</span>                                                                                                      <span class="err">21:43</span> <span class="err">Monday,</span> <span class="err">September</span> <span class="err">16,</span> <span class="err">2019</span><br><br><span class="s">29         ods listing close;ods html5 (id=saspy_internal) file=_tomods1 options(bitmap_mode=&#39;inline&#39;) device=svg style=HTMLBlue; ods graphics on /</span><br><span class="s">29       ! outputfmt=png;</span><br><span class="cm">NOTE: Writing HTML5(SASPY_INTERNAL) Body file: _TOMODS1</span><br><span class="s">30         </span><br><span class="s">31         </span><br><span class="s">32         %put Shows cell execution time</span><br><span class="s">33         </span><br><span class="s">34         ods html5 (id=saspy_internal) close;ods listing;</span><br><span class="err">Shows</span> <span class="err">cell</span> <span class="err">execution</span> <span class="err">time</span><br><span class="s">35         </span><br></pre></div>
</body>
</html>
...
- There are no wizards to generate code for you.  I don't that's impossible, it's just not there yet.  It might be possible to prompt for input using ipwidgets, but I have not looked into that.  You'd have to learn Python, but you already need to learn additional languages to create tasks in SAS Studio or EG.
...
- Jupyter is open source software, which means that the documentation is whatever someone feels like writing.  

<div style="page-break-after: always;"></div> 

## Appendix 1 : ~~Need to save and restore output~~ Displaying the SAS Log

>This appendix has been revised to use the %showLog line magic to display the SAS log.

If there are no errors in the log, and ODS output was created, you won't see the SAS log in Jupyter:
...
### Using the showLog cell magic to display the SAS log
...
<div class="highlight"><pre><span></span><span class="err">6</span> <span class="err">The</span> <span class="err">SAS</span> <span class="err">System</span>                                                                                                      <span class="err">21:43</span> <span class="err">Monday,</span> <span class="err">September</span> <span class="err">16,</span> <span class="err">2019</span><br><br><span class="s">48         ods listing close;ods html5 (id=saspy_internal) file=_tomods1 options(bitmap_mode=&#39;inline&#39;) device=svg style=HTMLBlue; ods graphics on /</span><br><span class="s">48       ! outputfmt=png;</span><br><span class="cm">NOTE: Writing HTML5(SASPY_INTERNAL) Body file: _TOMODS1</span><br><span class="s">49         </span><br><span class="s">50         proc sgplot data=SASHELP.CLASS;</span><br><span class="s">51             heatmap x=Height y=Weight;</span><br><span class="s">52         run;</span><br><br><span class="cm">NOTE: PROCEDURE SGPLOT used (Total process time):</span><br><span class="cm">      real time           0.43 seconds</span><br><span class="cm">      cpu time            0.08 seconds</span><br><span class="cm">      memory              7,988.25k</span><br><span class="cm">      OS Memory           30,776.00k</span><br><span class="cm">      Timestamp           2019-09-16 21:43:18 </span><br><span class="cm">      </span><br><span class="cm">NOTE: There were 19 observations read from the data set SASHELP.CLASS.</span><br><br><span class="s">53         </span><br><span class="s">54         ods html5 (id=saspy_internal) close;ods listing;</span><br><span class="s">55         </span><br></pre></div>
</body>
</html>



## Appendix 2 : Show both log and output by rerouting ODS output

>This section has been revised to reflect the use of the ``%showLog`` line magic, which is simpler.  The macros to defer printing ODS output in Jupyter might still be useful in some cases, and it's an interesting coding technique, so I'm leaving it in.

You can defer the display of ODS output in the log by rerouting it to an ODS document.  Because there is then no ODS output, Jupyter prints the SAS log.  You can then replay the ODS document to your Jupyter notebook.  
...
<div class="highlight"><pre><span></span><span class="err">7</span> <span class="err">The</span> <span class="err">SAS</span> <span class="err">System</span>                                                                                                      <span class="err">21:43</span> <span class="err">Monday,</span> <span class="err">September</span> <span class="err">16,</span> <span class="err">2019</span><br><br><span class="s">58         ods listing close;ods html5 (id=saspy_internal) file=_tomods1 options(bitmap_mode=&#39;inline&#39;) device=svg style=HTMLBlue; ods graphics on /</span><br><span class="s">58       ! outputfmt=png;</span><br><span class="cm">NOTE: Writing HTML5(SASPY_INTERNAL) Body file: _TOMODS1</span><br><span class="s">59         </span><br><span class="s">60         /* Macro to get list of open output destinations. */</span><br><span class="s">61         %macro util_return_open_ods_dests(delim= |, showdests=1, returndests=1);</span><br><span class="s">62         </span><br><span class="s">63             %let data = %UPCASE(sashelp.vdest);</span><br><span class="s">64         </span><br><span class="s">65             %local dsid ;</span><br><span class="s">66         </span><br><span class="s">67             /* Open the data set.  */</span><br><span class="s">68             %let dsid = %SYSFUNC(open(&amp;DATA., I));</span><br><span class="s">69         </span><br><span class="s">70             %if &amp;DSID.=0 %then</span><br><span class="s">71                 %do;</span><br><span class="s">72                     %put ERR%str()OR: Error opening dataset &amp;DATA..;</span><br><span class="s">73                     %put %sysfunc(sysmsg());</span><br><span class="s">74                     %return;</span><br><span class="s">75                 %end;</span><br><span class="s">76         </span><br><span class="s">77             /* Get variable numbers */</span><br><span class="s">78             %local dest_var_num style_var_num ;</span><br><span class="s">79         </span><br><span class="s">80             %let dest_var_num = %sysfunc(varnum(&amp;DSID. , DESTINATION));</span><br><span class="s">81             %if &amp;DEST_VAR_NUM.= 0 %then</span><br><span class="s">82                 %do;</span><br><span class="s">83                     %put ERR%str()OR: Variable not in data set.;</span><br><span class="s">84                     %put %sysfunc(sysmsg());</span><br><span class="s">85                     %goto exit;</span><br><span class="s">86                 %end;</span><br><span class="s">87         </span><br><span class="s">88             %let style_var_num = %sysfunc(varnum(&amp;DSID. , STYLE));</span><br><span class="s">89             %if &amp;STYLE_VAR_NUM.= 0 %then</span><br><span class="s">90                 %do;</span><br><span class="s">91                     %put ERR%str()OR: Variable not in data set.;</span><br><span class="s">92                     %put %sysfunc(sysmsg());</span><br><span class="s">93                     %goto exit;</span><br><span class="s">94                 %end;</span><br><span class="s">95         </span><br><span class="s">96             /* Get the values. */</span><br><span class="s">97         </span><br><span class="s">98             %local fetch_rc name_value style_value ;</span><br><span class="s">99         </span><br><span class="s">100            %let fetch_rc = 0;</span><br><span class="s">101        </span><br><span class="s">102            %do %until (&amp;FETCH_RC. ne 0);</span><br><span class="s">103                %let fetch_rc = %sysfunc(fetch(&amp;DSID., noset));</span><br><span class="s">104                %if &amp;FETCH_RC. ne 0 %then</span><br><span class="s">105                    %goto exit;</span><br><span class="s">106                %let name_value = %sysfunc(getvarc(&amp;DSID., &amp;DEST_VAR_NUM.));</span><br><span class="s">107                %let style_value = %sysfunc(getvarc(&amp;DSID., &amp;STYLE_VAR_NUM.));</span><br><span class="s">108                %if &amp;SHOWDESTS. %then</span><br><span class="s">109                    %put NOTE: ODS Destination &amp;NAME_VALUE. is open with style &amp;STYLE_VALUE.;</span><br><span class="s">110                %if &amp;RETURNDESTS. %then</span><br><span class="s">111                    %do;</span><br><span class="s">112                    &amp;DELIM.&amp;NAME_VALUE.</span><br><span class="s">113                %end;</span><br><span class="s">114            %end;</span><br><span class="s">115        </span><br><span class="s">116        %EXIT:</span><br><span class="s">117        </span><br><span class="s">118            %local close_rc ;</span><br><span class="s">119        </span><br><span class="s">120            %let close_rc = %SYSFUNC(close(&amp;DSID.));</span><br><span class="s">121        </span><br><span class="s">122            %if &amp;CLOSE_RC. ne 0 %then</span><br><span class="s">123                %do;</span><br><span class="s">124                    %put ERR%str()OR: Error closing dataset &amp;DATA..;</span><br><span class="s">125                    %put %sysfunc(sysmsg());</span><br><span class="s">126                %end;</span><br><span class="s">127        </span><br><span class="s">128            %return;</span><br><span class="s">129        </span><br><span class="s">130        %mend util_return_open_ods_dests;</span><br><span class="s">131        </span><br><span class="s">132        </span><br><span class="s">133        /* Macro to send default output to ODS document */</span><br><span class="s">134        %macro util_reroute_ods_to_document(name=work.__thisdoc, delim=|);</span><br><span class="s">135        </span><br><span class="s">136            %local i outcount outdest outlist;</span><br><span class="s">137        </span><br><span class="s">138            %let name = %upcase(&amp;NAME.);</span><br><span class="s">139        </span><br><span class="s">140            %put NOTE: ODS Output will be rerouted to ODS Document &amp;NAME. ;</span><br><span class="s">141        </span><br><span class="s">142            %let outlist = %util_return_open_ods_dests(delim=&amp;DELIM.);</span><br><span class="s">143        </span><br><span class="s">144            %let outcount = %sysfunc(countw(%superq(outlist), %superq(delim)));</span><br><span class="s">145        </span><br><span class="s">146            %if &amp;OUTCOUNT. = 0 %then</span><br><span class="s">147                %do;</span><br><span class="s">148                %put NOTE: No ODS Destinations active.;</span><br><span class="s">149                %return;</span><br><span class="s">150                %end;</span><br><span class="s">151        </span><br><span class="s">152            %do i = 1 %to &amp;OUTCOUNT.;</span><br><span class="s">153                %let outdest = %scan(%superq(OUTLIST), &amp;I., %superq(DELIM));</span><br><span class="s">154                %if &quot;&amp;OUTDEST.&quot; ne &quot;DOCUMENT&quot; %then</span><br><span class="s">155                    %do;</span><br><span class="s">156                        %put NOTE: ods &amp;OUTDEST. exclude all;</span><br><span class="s">157                        ods &amp;OUTDEST. exclude all;</span><br><span class="s">158                    %end;</span><br><span class="s">159            %end;</span><br><span class="s">160        </span><br><span class="s">161            %put NOTE: ods document name=&amp;NAME. (write);</span><br><span class="s">162            ods document name=&amp;NAME. (write);</span><br><span class="s">163        </span><br><span class="s">164        %mend util_reroute_ods_to_document;</span><br><span class="s">165        </span><br><span class="s">166        /* Macro to replay ODS document. */</span><br><span class="s">167        %macro util_replay_ods_document(name=work.__thisdoc, delim=|);</span><br><span class="s">168        </span><br><span class="s">169            %local i outcount outdest outlist;</span><br><span class="s">170        </span><br><span class="s">171            %let name = %upcase(&amp;NAME.);</span><br><span class="s">172        </span><br><span class="s">173            %put NOTE: ODS Output will be reprinted from ODS Document &amp;NAME. ;</span><br><span class="s">174        </span><br><span class="s">175            %let outlist = %util_return_open_ods_dests(delim=&amp;DELIM.);</span><br><span class="s">176        </span><br><span class="s">177            %let outcount = %sysfunc(countw(%superq(outlist), %superq(delim)));</span><br><span class="s">178        </span><br><span class="s">179            %if &amp;OUTCOUNT. = 0 %then</span><br><span class="s">180                %do;</span><br><span class="s">181                %put NOTE: No ODS Destinations active.;</span><br><span class="s">182                %return;</span><br><span class="s">183                %end;</span><br><span class="s">184        </span><br><span class="s">185            %do i = 1 %to &amp;OUTCOUNT.;</span><br><span class="s">186                %let outdest = %scan(%superq(OUTLIST), &amp;I., %superq(DELIM));</span><br><span class="s">187                %if &quot;&amp;OUTDEST.&quot; ne &quot;DOCUMENT&quot; %then</span><br><span class="s">188                    %do;</span><br><span class="s">189                        %put NOTE: ods &amp;OUTDEST. select all;</span><br><span class="s">190                        ods &amp;OUTDEST. select all;</span><br><span class="s">191                    %end;</span><br><span class="s">192            %end;</span><br><span class="s">193        </span><br><span class="s">194            ods document close;</span><br><span class="s">195        </span><br><span class="s">196            proc document name=&amp;NAME.;</span><br><span class="s">197                replay;</span><br><span class="s">198                run;</span><br><span class="s">199            quit;</span><br><span class="s">200        </span><br><span class="s">201        %mend util_replay_ods_document;</span><br><span class="s">202        </span><br><span class="s">203        ods html5 (id=saspy_internal) close;ods listing;</span><br><span class="s">204        </span><br></pre></div>
</body>
</html>
...
<div class="highlight"><pre><span></span><span class="err">8</span> <span class="err">The</span> <span class="err">SAS</span> <span class="err">System</span>                                                                                                      <span class="err">21:43</span> <span class="err">Monday,</span> <span class="err">September</span> <span class="err">16,</span> <span class="err">2019</span><br><br><span class="s">207        ods listing close;ods html5 (id=saspy_internal) file=_tomods1 options(bitmap_mode=&#39;inline&#39;) device=svg style=HTMLBlue; ods graphics on /</span><br><span class="s">207      ! outputfmt=png;</span><br><span class="cm">NOTE: Writing HTML5(SASPY_INTERNAL) Body file: _TOMODS1</span><br><span class="s">208        </span><br><span class="s">209        /* No log was produced above.  If you want both log and output, reroute the output and play it back separately. */</span><br><span class="s">210        </span><br><span class="s">211        %util_reroute_ods_to_document();</span><br><span class="cm">NOTE: ODS Output will be rerouted to ODS Document WORK.__THISDOC</span><br><span class="cm">NOTE: ODS Destination HTML5(SASPY_INTERNAL) is open with style HTMLBlue</span><br><span class="cm">NOTE: ods HTML5(SASPY_INTERNAL) exclude all</span><br><span class="cm">NOTE: ods document name=WORK.__THISDOC (write)</span><br><span class="s">212        </span><br><span class="s">213        proc sgplot data=SASHELP.CLASS;</span><br><span class="s">214            heatmap x=Height y=Weight / name=&#39;HeatMap&#39;;</span><br><span class="s">215            gradlegend &#39;HeatMap&#39;;</span><br><span class="s">216        run;</span><br><br><span class="cm">NOTE: PROCEDURE SGPLOT used (Total process time):</span><br><span class="cm">      real time           0.01 seconds</span><br><span class="cm">      cpu time            0.02 seconds</span><br><span class="cm">      memory              792.75k</span><br><span class="cm">      OS Memory           24,240.00k</span><br><span class="cm">      Timestamp           2019-09-16 21:43:21 </span><br><span class="cm">      </span><br><span class="cm">NOTE: There were 19 observations read from the data set SASHELP.CLASS.</span><br><br><span class="s">217        </span><br><span class="s">218        /* Log will appear below */</span><br><span class="s">219        </span><br><span class="s">220        ods html5 (id=saspy_internal) close;ods listing;</span><br><span class="s">221        </span><br></pre></div>
</body>
</html>
...
>NOTE: Shortly after I presented this paper, Jupyter Lab was updated from verasion 0.3 to 1.0.  That's a good thing in the long run, but most of the extensions stopped working.  Comments on GitHub indicate that this happens with many major updates, and that the extensions will come back in a few weeks after developers have had a chance to modify their programs.

>For now, the sections on installing Anaconda and the SAS kernel should work, but the extensions might ot might not work.
